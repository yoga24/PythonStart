...
# Replace <<GDRIVE_FOLDER_URL>>, <<USER_MAIL>>, <<PASSWORD>> with appropriate values.
...
if 'accounts.google.com/signin' in browser.url:
    ...
    pwd = browser.find_by_xpath('//input[@name="password"]')[0]
    pwd_next = browser.find_by_xpath('//*[@id="passwordNext"]')[0]

    pwd.fill('<<PASSWORD>>')
    pwd_next.click()

time.sleep(20)  # To make sure all elements are loaded before searching using XPath
